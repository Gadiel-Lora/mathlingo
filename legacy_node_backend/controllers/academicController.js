...
import { attachRoleContext, canAccessUserId, requireAdmin, requireAuth, requireStaff } from '../middlewares/authMiddleware.js'
...
const router = Router()
const withRoleContext = [requireAuth, attachRoleContext]
const withStaffContext = [requireAuth, attachRoleContext, requireStaff]
const withAdminContext = [requireAuth, attachRoleContext, requireAdmin]

const sanitizeInput = (value) => String(value ?? '').trim()

const resolveScopedUserId = (req, res, rawUserId, options = {}) => {
  const requestedUserId = sanitizeInput(rawUserId)
  const targetUserId = requestedUserId || (options.fallbackToCurrentUser === false ? '' : sanitizeInput(req.user?.id))

  if (!targetUserId) {
    res.status(400).json({
      error: options.requiredMessage || 'userId es obligatorio.',
    })
    return null
  }

  if (!canAccessUserId(req, targetUserId, { allowStaff: options.allowStaff !== false })) {
    res.status(403).json({
      error: options.forbiddenMessage || 'No tienes permisos para acceder al progreso de otro usuario.',
    })
    return null
  }
...
router.post('/skills/unlocked', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
...
router.post('/skills/next', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const domain = sanitizeInput(req.body?.domain)
  const grade = parseGrade(req.body?.grade)
...
router.post('/mastery/state', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  if (!userId) return
...
router.post('/curriculum/lesson/completed', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const lessonId = sanitizeInput(req.body?.lessonId)
  if (!userId || !lessonId) {
    res.status(400).json({
...
router.post('/curriculum/unit/completed', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const unitId = sanitizeInput(req.body?.unitId)
  if (!userId || !unitId) {
    res.status(400).json({
      error: 'unitId es obligatorio.',
    })
    return
  }
...
router.post('/adaptive/recommendation', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const grade = parseGrade(req.body?.grade)
  const completedSkills = parseCompletedSkills(req.body?.completedSkills)
...
router.post('/evaluation/generate', ...withRoleContext, (req, res) => {
  const grade = parseGrade(req.body?.grade)
...
router.post('/question/generate', ...withRoleContext, async (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const grade = parseGrade(req.body?.grade)
  ...
  if (!grade || !topic) {
    res.status(400).json({
      error: 'grade y topic son obligatorios.',
    })
    return
  }
...
router.post('/question/state', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  ...
  if (!questionHash) {
    res.status(400).json({
      error: 'questionHash es obligatorio.',
    })
    return
  }
...
router.post('/question/help', ...withRoleContext, async (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const questionHash = sanitizeInput(req.body?.questionHash)
...
router.post('/question/chat', ...withRoleContext, async (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const questionHash = sanitizeInput(req.body?.questionHash)
...
router.post('/question/submit', ...withRoleContext, async (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const questionHash = sanitizeInput(req.body?.questionHash)
...
router.post('/question/reset', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  const questionHash = sanitizeInput(req.body?.questionHash)
...
router.post('/final-exam/generate', ...withRoleContext, (req, res) => {
  const grade = parseGrade(req.body?.grade)
  const userId = resolveScopedUserId(req, res, req.body?.userId)
...
router.post('/retention/profile', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  if (!userId) return
...
router.post('/retention/due', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
...
router.post('/analytics/student', ...withRoleContext, (req, res) => {
  const userId = resolveScopedUserId(req, res, req.body?.userId)
  if (!userId) return
...
router.post('/analytics/teacher', ...withStaffContext, (req, res) => {
...
router.get('/analytics/admin', ...withAdminContext, (req, res) => {
  res.status(200).json({
...
router.post('/predictive/outcomes', ...withRoleContext, (req, res) => {
  const rawUserId = sanitizeInput(req.body?.userId)
  const userId = rawUserId ? resolveScopedUserId(req, res, rawUserId) : ''
  if (rawUserId && !userId) return
...
router.post('/master-context', ...withRoleContext, (req, res) => {
  const rawUserId = sanitizeInput(req.body?.userId)
  const userId = rawUserId ? resolveScopedUserId(req, res, rawUserId) : sanitizeInput(req.user?.id)
  if (rawUserId && !userId) return
  const grade = parseGrade(req.body?.grade)
  const completedSkills = parseCompletedSkills(req.body?.completedSkills)
